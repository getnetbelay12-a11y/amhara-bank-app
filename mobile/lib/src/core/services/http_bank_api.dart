...
class HttpAuthApi implements AuthApi {
  ...
  Future<OnboardingStatus> getOnboardingStatus({
    ...
    return OnboardingStatus(
      ...
      lastUpdatedAt:
          DateTime.tryParse(data['lastUpdatedAt'] as String? ?? ''),
    );
  }
    ...
    return MemberSession(
      ...
      featureFlags: MemberFeatureFlags(
        voting: (user['featureFlags'] as Map<String, dynamic>?)?['voting']
            as bool? ??
            false,
        announcements: (user['featureFlags'] as Map<String, dynamic>?)?['announcements']
            as bool? ??
            false,
  ...
  Future<Map<String, dynamic>> uploadLoanDocument(
    ...
    final response = await _client.post(
      ...
      body: jsonEncode({
        ...
        if (storageKey != null && storageKey.isNotEmpty) 'storageKey': storageKey,
...
class HttpAutopayApi implements AutopayApi {
  ...
  AutopayInstruction _toInstruction(Map<String, dynamic> item) {
    return AutopayInstruction(
      id: item['id'] as String? ?? '',
      serviceType: item['provider'] as String? ?? item['serviceType'] as String? ?? 'autopay',
...
class HttpCardApi implements CardApi {
  ...
  CardItem _toCard(Map<String, dynamic> item) {
    return CardItem(
      ...
      channelControls: (item['channelControls'] as Map<String, dynamic>?) ?? const {},
  ...
  Future<VoteDetail> fetchVoteDetail(String voteId) async {
    ...
    return VoteDetail(
      ...
      startDate: DateTime.tryParse(data['startDate'] as String? ?? '') ??
          DateTime(2026, 1, 1),
      endDate:
          DateTime.tryParse(data['endDate'] as String? ?? '') ?? DateTime(2026, 1, 1),
      options: options,
    );
  }
