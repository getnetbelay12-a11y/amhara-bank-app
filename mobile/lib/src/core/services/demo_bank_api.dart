...
class DemoAuthApi implements AuthApi {
  ...
  Future<MemberSession> login({
    ...
    return MemberSession(
      ...
      customerId: isShareholder ? 'AMH-100001' : 'AMH-100003',
  ...
  Future<PinRecoveryOptions> getPinRecoveryOptions({
    ...
    if (normalized == '0911000001' ||
        normalized == 'selamawit.molla@amharabank.com') {
...
class DemoDocumentUploadApi implements DocumentUploadApi {
  ...
  Future<UploadedDocument> uploadDocument({
    ...
    final resolvedEntityId = entityId ?? 'upload_${DateTime.now().millisecondsSinceEpoch}';
  ...
  SmartInsightFeed _buildFeed() {
    ...
    final items = [
      ...
      SmartInsight(
        ...
        message: 'Your recent deposits look healthy. Consider moving some funds into savings.',
  ...
  Future<List<LoanTimelineItem>> fetchLoanTimeline(String loanId) async {
    ...
    return [
      const LoanTimelineItem(
        status: 'submitted',
        title: 'Submitted',
        description: 'Your application was received and entered into the queue.',
        isCompleted: true,
        isCurrent: false,
      ),
      LoanTimelineItem(
        ...
        title: 'Branch Review',
        description:
            loan.deficiencyReasons.isNotEmpty
        ...
        isCompleted: loan.currentLevel != 'branch' || loan.status != 'submitted',
      ...
      LoanTimelineItem(
        ...
        isCompleted: ['head_office', 'approved', 'disbursed', 'rejected']
            .contains(loan.currentLevel) || ['approved', 'disbursed', 'rejected'].contains(loan.status),
        isCurrent: loan.currentLevel == 'district' && loan.status == 'submitted',
      ),
      LoanTimelineItem(
        ...
        isCompleted: ['approved', 'disbursed', 'rejected'].contains(loan.status),
        isCurrent: loan.currentLevel == 'head_office' && loan.status == 'submitted',
      ),
      LoanTimelineItem(
        ...
        isCurrent: loan.deficiencyReasons.isNotEmpty && loan.status == 'submitted',
      ),
      LoanTimelineItem(
        status: 'approved',
        title: 'Approved',
...
class DemoNotificationApi implements NotificationApi {
  final List<AppNotification> _items = [
    AppNotification(
      ...
      message: 'A new login to your Amhara Bank mobile profile was detected.',
    ...
    AppNotification(
      ...
      message: 'Your ATM card request is under review. Tap to open the request timeline.',
    ...
    AppNotification(
      ...
      status: 'sent',
      title: 'Support Reply',
      message: 'An Amhara Bank support agent replied to your request.',
...
class DemoAutopayApi implements AutopayApi {
  ...
  Future<AutopayInstruction> createInstruction({
    ...
    await Future<void>.delayed(const Duration(milliseconds: 120));
    final existingIndex = _items.indexWhere((item) => item.serviceType == provider);
  ...
  Future<AutopayInstruction> updateInstructionStatus({
    ...
    await Future<void>.delayed(const Duration(milliseconds: 120));
    final index = _items.indexWhere(
      (item) => (id != null && item.id == id) || (provider != null && item.serviceType == provider),
    );
    if (index < 0) {
  ...
  final List<ChatConversation> _conversations = [
    ...
    ChatConversation(
      ...
      latestMessage: ChatMessage(
        ...
        senderName: 'Amhara Bank Assistant',
        message:
  ...
  Future<ChatConversation> createConversation({
    ...
    final created = ChatConversation(
      ...
      latestMessage: ChatMessage(
        ...
        senderName: 'Amhara Bank Assistant',
        message:
            'Welcome to Amhara Bank support. A smart assistant is here first and an agent can join if needed.',
        messageType: 'system',
        createdAt: now,
      ),
      messages: [
...
class DemoServiceRequestApi implements ServiceRequestApi {
  ...
  Future<List<ServiceRequest>> fetchMyRequests() async {
    return [
      ServiceRequest(
        ...
        description: 'A transfer was debited but has not reached the destination account.',
        status: 'under_review',
      ...
      ServiceRequest(
        ...
        description: 'Requested replacement of the phone number linked to the account.',
        status: 'awaiting_customer',
      ...
      ServiceRequest(
        ...
        description: 'Requested spouse relationship update for joint account servicing.',
        status: 'under_review',
        latestNote: 'Relationship evidence is under branch review.',
        createdAt: DateTime(2026, 3, 8, 10, 5),
        payload: const {
          'relationshipType': 'spouse',
          'relatedMemberNumber': 'AMH-100112',
          'relatedCustomerId': 'AMH-100112',
        },
  ...
  Future<ServiceRequest> fetchRequestDetail(String requestId) async {
    ...
    return items.firstWhere((item) => item.id == requestId, orElse: () => items.first);
  }
