...
describe('PaymentDisputesPage', () => {
  ...
  it('filters to payment-related service requests and updates a dispute status', async () => {
    ...
    expect(await screen.findByText('Abebe Kebede (AMH-100001)')).toBeInTheDocument();
    expect(screen.queryByText('Meseret Alemu (AMH-100002)')).not.toBeInTheDocument();
    ...
    expect(screen.getAllByText('Payment Dispute')).toHaveLength(2);
    ...
    expect(await screen.findByText('Updated payment case AMH-100001 to Completed.')).toBeInTheDocument();
  });
});
