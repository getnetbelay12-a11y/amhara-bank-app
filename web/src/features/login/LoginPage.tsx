...
function getGreeting(name: string) {
  ...
  return `${salutation}. Secure access is limited to your assigned Amhara Bank scope.`;
}
...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const greeting =
    selectedRole === 'school'
      ...
      : getGreeting('Staff');
  const heroEyebrow =
    selectedRole === 'school' ? 'School Console Access' : 'Amhara Bank Staff Access';
  const heroTitle =
    ...
    <div className="standalone-page">
      <div className="login-layout">
        <section className="login-hero">
          ...
          <div className="login-brand-ribbon">AMHARA BANK</div>
          <h1>{heroTitle}</h1>
            ...
            onSubmit={async (event) => {
              ...
              try {
                ...
                if (!isSchoolSession(session)) {
                  window.sessionStorage.setItem('amhara_demo_role', session.role);
                }
                onLogin(session);
              } catch (_error) {
