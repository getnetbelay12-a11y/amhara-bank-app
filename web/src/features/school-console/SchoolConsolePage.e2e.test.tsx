...
describe('school console E2E flow', () => {
  ...
  it('registers a student with parent detail and linked member access', async () => {
    ...
    await user.type(registrationQueries.getByLabelText('Student ID'), 'ST-4999');
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/linked to AMH-100001 from the parent phone number/i)).toBeInTheDocument();
    });

    await user.click((await screen.findAllByRole('button', { name: 'Parent Linking' }))[0]);
    await user.clear(screen.getByLabelText('Search'));
    await user.type(screen.getByLabelText('Search'), 'ST-4999');

    let registryRow: HTMLElement | undefined;
    await waitFor(() => {
      registryRow = screen
        .getAllByRole('row')
        .find((candidate) => candidate.textContent?.includes('Abel Getnet (ST-4999)'));
    ...
    expect(rowQueries.getByText('Abel Getnet (ST-4999)')).toBeInTheDocument();
    ...
    expect(detailQueries.getAllByText('AMH-100001').length).toBeGreaterThan(0);
