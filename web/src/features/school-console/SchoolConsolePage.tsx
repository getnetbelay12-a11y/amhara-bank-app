...
export function SchoolConsolePage({
  ...
  const consoleDescription = isSchoolPortal
    ...
    : 'Bank-side workspace for school onboarding, billing control, collections, and partner-school reconciliation.';
  ...
  const overviewDescription = isSchoolPortal
    ...
    : 'Head-office and branch teams can onboard schools, monitor collections, and spot reconciliation pressure.';
  ...
  async function handleRegisterStudent(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      setSetupMessage('Student ID, student name, grade, parent name, and phone are required.');
      return;
    }
    ...
    await schoolConsoleApi.importStudents({
      ...
      students: [
        {
          studentId: registrationForm.studentId.trim(),
    ...
    const [nextOverview, nextRegistry, nextFeePlans, detail] = await Promise.all([
      ...
      schoolConsoleApi.getStudentDetail(registrationForm.studentId.trim()),
    ]);

    setOverview(nextOverview);
    setRegistry(nextRegistry);
    setFeePlans(nextFeePlans);
    setSelectedStudentId(registrationForm.studentId.trim());
    ...
    setSetupMessage(
      `Registered ${detail?.student.fullName ?? 'student'} with parent contact${
        ...
        linkedMemberAccount?.customerId
          ? linkedMemberAccount.resolvedByPhone
            ...
            : '; the student is ready to appear in the parent app.'
        ...
        >
          <div className="loan-detail-grid">
            <form className="form-stack" onSubmit={(event) => void handleRegisterStudent(event)}>
              ...
              <label className="field-stack">
                <span>Member customer ID if known (optional, phone number can auto-link)</span>
                <input
