...
export class EmailNotificationProvider implements ChannelNotificationProvider {
  ...
  async send(payload: ChannelNotificationPayload): Promise<ChannelNotificationResult> {
    ...
    if (config.provider === 'log') {
      ...
      this.logger.log(
        `Email reminder queued in log mode recipient=${payload.recipient} subject="${payload.subject ?? 'Amhara Bank Notification'}" messageId=${messageId} attachments=${payload.attachments?.length ?? 0}`,
      );
      return {
        status: 'sent',
    ...
    try {
      ...
      const info = await transport.sendMail({
        from: `Amhara Bank <${config.sender}>`,
        to: payload.recipient,
        subject: payload.subject ?? 'Amhara Bank Notification',
