...
export class TelegramSubscriptionService {
  ...
  async createLinkCode(
    ...
    if (emailRecipient) {
      const emailResult = await this.emailNotificationProvider.send({
        ...
        subject: 'Your Amhara Bank Telegram Link Code',
        messageBody: [
          `Hello ${member.fullName},`,
          '',
          'Use the following code to link your Telegram bot subscription to your Amhara Bank customer profile:',
          code,
          '',
          'Open the Amhara Bank Telegram bot and send:',
          `/link ${code}`,
          '',
    ...
    return {
      ...
      instructions:
        'Open the Amhara Bank Telegram bot and send /link <code> to connect your account.',
    };
  }
  ...
  async handleWebhook(
    ...
    if (text.startsWith('/start ') || text.startsWith('/link ')) {
      ...
      if (!code) {
        await this.reply(chatId, 'Please send /link <code> with the code generated in the Amhara Bank app.');
  ...
  private async linkChatId(
    ...
    if (!linkCode) {
      await this.reply(
        input.chatId,
        'This link code is invalid or expired. Generate a new Telegram link code in the Amhara Bank app and try again.',
      );
    ...
    await this.reply(
      input.chatId,
      `Amhara Bank Telegram reminders are now linked to customer ${member.customerId}. Loan and insurance reminders will be sent here while your subscription remains active.`,
    );
  ...
  private async updateSubscriptionFromCommand(
    ...
    if (!member) {
      await this.reply(
        chatId,
        'This Telegram chat is not linked yet. Generate a link code in the Amhara Bank app and send /link <code> first.',
      );
  ...
  private startMessage() {
    return [
      'Welcome to Amhara Bank Telegram reminders.',
      '',
      'To link your bank profile, open the Amhara Bank app and generate a Telegram link code, then send:',
      '/link YOURCODE',
      '',
      'Available commands:',
      '/unsubscribe',
  ...
  private helpMessage() {
    return [
      'Amhara Bank Telegram reminders',
      '',
