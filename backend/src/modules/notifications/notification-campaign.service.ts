...
export class NotificationCampaignService {
  ...
  private async resolveTargets(
    ...
    return this.shouldLimitToSingleEmailTarget(dto) ? members.slice(0, 1) : members;
  }
  ...
  private shouldLimitToSingleEmailTarget(dto: CreateNotificationCampaignDto) {
    return (
      this.normalizeChannels(dto.channels).includes(NotificationChannel.EMAIL) &&
      Boolean(this.resolveForcedEmailRecipient(dto.demoRecipientEmail))
    );
  }
