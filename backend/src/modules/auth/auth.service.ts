...
export class AuthService implements OnModuleInit {
    ...
    const deliveryResult =
      channel === NotificationChannel.EMAIL
        ? await this.emailNotificationProvider.send({
            ...
            subject: 'Amhara Bank OTP Verification Code',
            messageBody: `Your Amhara Bank verification code is ${otpCode}. It expires soon.`,
            htmlBody: `<p>Your Amhara Bank verification code is <strong>${otpCode}</strong>.</p><p>It expires soon.</p>`,
          })
        : await this.smsNotificationProvider.send({
            ...
            memberId: 'otp-pre-registration',
            category: NotificationCategory.LOAN,
            messageBody: `Amhara Bank OTP: ${otpCode}. It expires soon.`,
          });
  ...
  private async generateStandardIdentifiers() {
    ...
    return {
      customerId: `AMH-${sequence}`,
      memberId: `AMH-${sequence}`,
    };
  }
  ...
  private async ensureDemoMemberAccount() {
    ...
    this.logger.log(
      `Ensured seeded member account customerId=AMH-100001 memberNumber=AMH-100001 phone=${demoPhone}`,
    );
  }
