...
describe('StorageService', () => {
  ...
  it('writes binary document bytes to local storage', async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), 'amhara-upload-'));
    const configService = {
      getOrThrow: jest.fn().mockReturnValue({
        provider: 'local',
        uploadPath: tempDir,
  ...
  it('reads stored binary documents and preserves metadata', async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), 'amhara-upload-read-'));
    const configService = {
      getOrThrow: jest.fn().mockReturnValue({
        provider: 'local',
        uploadPath: tempDir,
  ...
  it('reads stored document metadata without loading content into callers', async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), 'amhara-upload-meta-'));
    const configService = {
      getOrThrow: jest.fn().mockReturnValue({
        provider: 'local',
        uploadPath: tempDir,
